...
void CDRMAtomic::DrmAtomicCommit(int fb_id, int flags, bool rendered, bool videoLayer)
{
  ...
  if (ret < 0)
  {
    CLog::LogF(LOGERROR, "atomic commit failed: {}", __FUNCTION__, strerror(errno));
  ...
  if (flags & DRM_MODE_ATOMIC_ALLOW_MODESET)
  {
    if (drmModeDestroyPropertyBlob(m_fd, blob_id) != 0)
      CLog::LogF(LOGERROR, "failed to destroy property blob: {}", __FUNCTION__, strerror(errno));
  }
...
void CDRMAtomic::FlipPage(struct gbm_bo* bo, bool rendered, bool videoLayer, bool async)
{
  ...
  if (rendered)
  {
    ...
    if (!drm_fb)
    {
      CLog::LogF(LOGERROR, "Failed to get a new FBO", __FUNCTION__);
      return;
    }
  ...
  if (m_need_modeset)
  {
    ...
    CLog::LogF(LOGDEBUG, "Execute modeset at next commit", __FUNCTION__);
  }
...
bool CDRMAtomic::InitDrm()
{
  ...
  if (ret)
  {
    CLog::LogF(LOGERROR, "no atomic modesetting support: {}", __FUNCTION__, strerror(errno));
    return false;
  }
  ...
  CLog::LogF(LOGDEBUG, "initialized atomic DRM", __FUNCTION__);
                                                  ...
                                                  CDRMAtomicRequest* old)
{
  ...
  CLog::LogF(LOGDEBUG, "DRM Atomic Request Diff:", __FUNCTION__);
...
{
  CLog::LogF(LOGDEBUG, "DRM Atomic Request:", __FUNCTION__);
