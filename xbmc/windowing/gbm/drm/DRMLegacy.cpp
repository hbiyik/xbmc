...
bool CDRMLegacy::SetVideoMode(const RESOLUTION_INFO& res, struct gbm_bo* bo)
{
  ...
  if (ret < 0)
  {
    CLog::LogF(LOGERROR, "failed to set crtc mode: {}x{}{} @ {} Hz", __FUNCTION__, m_mode->hdisplay,
  ...
  CLog::LogF(LOGDEBUG, "set crtc mode: {}x{}{} @ {} Hz", __FUNCTION__, m_mode->hdisplay,
             m_mode->vdisplay, m_mode->flags & DRM_MODE_FLAG_INTERLACE ? "i" : "",
             m_mode->vrefresh);
...
bool CDRMLegacy::QueueFlip(struct gbm_bo* bo)
{
  ...
  if (ret)
  {
    CLog::LogF(LOGDEBUG, "failed to queue DRM page flip", __FUNCTION__);
    return false;
  }
...
bool CDRMLegacy::InitDrm()
{
  ...
  CLog::LogF(LOGDEBUG, "initialized legacy DRM", __FUNCTION__);
  return true;
}
...
bool CDRMLegacy::SetActive(bool active)
{
  ...
  {
    CLog::LogF(LOGDEBUG, "failed to set DPMS property", __FUNCTION__);
    return false;
  }
