...
bool CDRMPlane::SupportsFormatAndModifier(uint32_t format, uint64_t modifier)
{
  ...
  if (modifier == DRM_FORMAT_MOD_LINEAR)
  {
    ...
    {
      CLog::LogF(LOGDEBUG, "format not supported: {}", __FUNCTION__,
  ...
  else
  {
    ...
    {
      CLog::LogF(LOGDEBUG, "modifier ({}) not supported for format ({})", __FUNCTION__,
                 DRMHELPERS::ModifierToString(modifier), DRMHELPERS::FourCCToString(format));
      return false;
    }
  }

  CLog::LogF(LOGDEBUG, "found plane format ({}) and modifier ({})", __FUNCTION__,
...
bool CDRMPlane::MoveOnTopOf(CDRMPlane* other)
{
  ...
  CLog::LogF(LOGDEBUG, "zpos_available: {}", __FUNCTION__, zpos_available);
  CLog::LogF(LOGDEBUG, "plane {} zpos: {}[{}-{}], immutable: {}", __FUNCTION__, GetId(), zpos,
             zpos_min, zpos_max, zpos_immutable);
  CLog::LogF(LOGDEBUG, "plane {} zpos: {}[{}-{}], immutable: {}", __FUNCTION__, other->GetId(),
             other_zpos, other_zpos_min, other_zpos_max, other_zpos_immutable);
  ...
  if (!other_zpos_immutable && zpos_immutable && zpos > other_zpos_min)
  {
    ...
    CLog::LogF(LOGDEBUG, "moving down plane {}, zpos [{}->{}] under plane {} zpos {}", __FUNCTION__,
  ...
  if (other_zpos_immutable && !zpos_immutable && zpos_max > other_zpos)
  {
    ...
    {
      CLog::LogF(LOGDEBUG, "moving up plane {}, zpos [{}->{}] on top of plane {} zpos {}",
  ...
  if (!other_zpos_immutable && !zpos_immutable && zpos_max > other_zpos_min)
  {
    ...
    {
      CLog::LogF(LOGDEBUG,
                 "moving up plane {} zpos [{}->{}] and moving down plane {} zpos [{}->{}]",
