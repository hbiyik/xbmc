...
bool CDRMUtils::FindGuiPlane(uint32_t format, uint64_t modifier)
{
  ...
  for (auto& crtc : m_encoder->GetPossibleCrtcs(m_crtcs, m_crtc))
  {
    for (auto& plane : m_planes)
    {
      ...
      m_gui_format = *gui_format;
      m_gui_modifier = modifier;
      CLog::Log(LOGINFO,
                "CDRMUtils::{} - Using gui plane [{}x{}] id:{}, format:{}, modifier:{}, crtc id:{}",
                __FUNCTION__, res.iWidth, res.iHeight, m_gui_plane->GetId(),
                DRMHELPERS::FourCCToString(m_gui_format.drm),
                DRMHELPERS::ModifierToString(m_gui_modifier), m_crtc->GetId());
      return true;
    }
  }
                                     ...
                                     uint64_t height)
{
  ...
  for (auto& crtc : m_encoder->GetPossibleCrtcs(m_crtcs, m_crtc))
  {
    ...
    {
      if (!gui_plane->Check(res.iWidth, res.iHeight, m_gui_format.drm, m_gui_modifier, crtc,
                            gui_type))
        continue;
  ...
  CLog::Log(LOGWARNING,
            ...
            DRMHELPERS::FourCCToString(m_gui_format.drm),
            DRMHELPERS::ModifierToString(m_gui_modifier));
  return false;
}
